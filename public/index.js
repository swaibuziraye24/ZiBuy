...
// ── TEMP TEST — triggers actual reminder creation ──
exports.runBoostRemindersNow = regionalFunctions.https.onRequest(async (req, res) => {
  const now   = new Date();
  const in48h = new Date(now.getTime() + 48 * 60 * 60 * 1000);
  ...
  try {
    ...
    for (const docSnap of snap.docs) {
      ...
      if (phone) {
        const waMsg = encodeURIComponent(
          ...
          `• 14 Days — UGX 8,000\n` +
          `• 30 Days — UGX 15,000\n\n` +
          `📱 MTN: Dial *165# → Merchant: 27868095\n` +
          `📱 Airtel: Send to +256575996624\n\n` +
          `ZiBuy Uganda 🛍️`
        );
    ...
    res.json({
      message: `Done! Created ${created} reminders`,
      checked: snap.size
    });
